...
## Why AI generation is deferred
- MVP favors deterministic, reviewable reports derived from user-approved entries. Automated summarization/analysis is deferred; optional audio transcription is available via the backend.

## Development notes
- Services are isolated via Docker Compose; internal hostnames follow service names (e.g., `mongo`, `minio`, `backend`).
- JWT middleware guards all routes except `/auth/login`, `/health`, and `/media/:objectName`; a seeded engineer account simplifies first-run access.
